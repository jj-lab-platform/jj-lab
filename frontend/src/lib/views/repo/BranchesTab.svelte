...
<div class="g-card overflow-hidden">
  {#each app.branches as b (b.name)}
    <div class="flex w-full items-center gap-3 border-b border-[var(--gitea-secondary)] px-4 py-2.5 last:border-b-0">
      <a href={`#/${encodeURIComponent(app.route.org!)}/${encodeURIComponent(app.route.repo!)}`} onclick={() => { app.branch = b.name; void app.loadTree() }}
      ...
      <button class="shrink-0 font-mono text-[11px] hover:underline" style="color:var(--destructive)" onclick={() => void app.doDeleteBranch(b.name)}>delete</button>
    </div>
  {/each}
</div>
